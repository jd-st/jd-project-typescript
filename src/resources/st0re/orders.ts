...
export class Orders extends APIResource {
  ...
  create(
    ...
    return this._client.post('/st0re/order', { body, ...options });
  }
   ...
   * @example
   * ```ts
   * const order = await client.st0re.orders.retrieve(0);
   * ```
   */
  retrieve(orderID: number, options?: RequestOptions): APIPromise<Shared.Order> {
    return this._client.get(path`/st0re/order/${orderID}`, options);
  }
   ...
   * @example
   * ```ts
   * await client.st0re.orders.delete(0);
   * ```
   */
  delete(orderID: number, options?: RequestOptions): APIPromise<void> {
    return this._client.delete(path`/st0re/order/${orderID}`, {
      ...options,
